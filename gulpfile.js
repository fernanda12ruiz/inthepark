...
gulp.task('bower:css', function() {
    var options = settings.bower.css;
...
gulp.task('bower:fonts', function() {
    var options = settings.bower.fonts;
    gulp.src(bowerFiles())
        .pipe(p.filter(options.filter))
        .pipe(gulp.dest(options.dest));
});
...
gulp.task('app:templates', function() {
    var options = settings.app.templates;

    appTemplates = gulp.src(options.src)
        .pipe(p.minifyHtml({ empty: true }))
        .pipe(p.angularTemplatecache({ module: 'esqueleto' }))
        .pipe(p.concat(options.compileFileName))
        .pipe(p.uglify({
            mangle: false
        }))
...
gulp.task('app:images', function() {
    var options = settings.app.images;
...
gulp.task('run', ['app:images', 'inject', 'watch'], function() {
    browserSync.init({
        server: {
