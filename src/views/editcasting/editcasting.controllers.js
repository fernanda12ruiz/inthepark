...
angular.module('esqueleto')
    .controller('editcastingController', function($scope, $routeParams, $route, $firebaseObject, $firebaseArray) {
        'use strict';
        ...
        $scope.casting = $firebaseObject(ref.child('Portfolio/casting/' + $scope.id));
        $scope.addies = $firebaseArray(ref.child('Portfolio/casting/' + $scope.id + '/Addon'))
        $scope.talents = $firebaseArray(ref.child('Portfolio/casting/' + $scope.id + '/Featalent'))


        $scope.sosocial = $firebaseArray(ref.child('Portfolio/casting/' + $scope.id + '/Featalent'));
            ...
            task.on('state_changed', function progress(snapshot) {
                ...
                function complete() {
                    storageRef.getDownloadURL().then(function(url) {
                        ref.child('Portfolio/casting/' + $scope.id + '/Thumbnail').set(url);
                    });
        ...
        $scope.uHeader = function(e) {
            ...
            task.on('state_changed', function progress(snapshot) {
                ...
                function complete() {
                    storageRef.getDownloadURL().then(function(url) {
                        ref.child('Portfolio/casting/' + $scope.id + '/Header').set(url);
                    });
        ...
        $scope.newAddonImg = function() {
            ...
            ref.child('Portfolio/casting/' + $scope.id + '/Addon/').push({
                Order: numAddons + 1,
                Type: "imgvid"
            });
        }
        ...
        $scope.newFeatalent = function() {
            $firebaseArray(ref.child('Portfolio/casting/' + $scope.id + '/Featalent/').push({
                Type: "Featalent",
                TalentID: null
            }));
        };
        ...
        $scope.DelSoc = function(item) {
            ...
            try {
                ...
                $firebaseArray(ref.child('Portfolio/casting/' + $scope.id + '/Featalent/' + SID).set(null));
                $scope.talents.$save();
            }
                    ...
                    if ($scope.addies[i].Order) {
                        if ($scope.addies[i].Order == POrderM) {
                            try {
                                ...
                                $firebaseArray(ref.child('Portfolio/casting/' + $scope.id + '/Addon/' + OID + '/Order').set(POrder));
                                $scope.rndie();
                            } catch (e) {} finally {
                                $firebaseArray(ref.child('Portfolio/casting/' + $scope.id + '/Addon/' + PID + '/Order').set(POrderM));
                            }
                        }
                    }
                }
                    ...
                    if ($scope.addies[i].Order) {
                        if ($scope.addies[i].Order == POrderP) {
                            try {
                                ...
                                $firebaseArray(ref.child('Portfolio/casting/' + $scope.id + '/Addon/' + PID + '/Order').set(POrderP));
                            }
                        }
                    }
                }
        ...
        $scope.deleteAddonie = function(e, item) {
                    ...
                    if ($scope.addies[i].Order) {
                        if ($scope.addies[i].Order > thisOrder) {
                            ...
                            try {
                                ...
                                var RVAL = $scope.addies[i].Order;
                                var NVAL = RVAL - 1;
                                $firebaseArray(ref.child('Portfolio/casting/' + $scope.id + '/Addon/' + OID + '/Order').set(NVAL));
                            } catch (e) {} finally {
                                try {} catch (e) {}
                            }
                        }
            ...
            $firebaseArray(ref.child('Portfolio/casting/' + $scope.id + '/Addon/' + thisadd).set(null));
        }
        ...
        $scope.ChangeIMG = function(e) {
            ...
            task.on('state_changed', function progress(snapshot) {
                ...
                function complete() {
                    storageRef.getDownloadURL().then(function(url) {
                        ref.child('Portfolio/casting/' + $scope.id + '/Addon/' + THEID + '/Src').set(url);
                        ref.child('Portfolio/casting/' + $scope.id + '/Addon/' + THEID + '/Tipo').set(tipo);
                    });
