...
angular.module('esqueleto')
    .controller('newpageController', function ($scope, $routeParams, $route, $firebase, $firebaseObject, $firebaseArray) {
    'use strict';
    ...
    $scope.HITHERE = function(){
        $scope.PLENGHT = $scope.page.length + 1;
    }
    $scope.waitSave = 0;
    ...
    $scope.newP = function(){
        ...
        if (!$scope.pro.Url) {
            ...
            $scope.pro.Order = $scope.page.length + 1;
            ref.child('Portfolio/page/'+$scope.pro.Url).set($scope.pro).then(function(val) {
                location.href = '/cccpage';
            });
        }
    };
