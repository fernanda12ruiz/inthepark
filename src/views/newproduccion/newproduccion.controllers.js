...
angular.module('esqueleto')
    .controller('newproduccionController', function ($scope, $routeParams, $route, $firebase, $firebaseObject, $firebaseArray) {
    'use strict';
    ...
    $scope.HITHERE = function(){
        $scope.PLENGHT = $scope.produccion.length + 1;
    }
    $scope.waitSave = 0;
    ...
    $scope.newP = function(){
        ...
        if (!$scope.pro.Url) {
            ...
            $scope.pro.Order = $scope.produccion.length + 1;
            ref.child('Portfolio/produccion/'+$scope.pro.Url).set($scope.pro).then(function(val) {
                location.href = '/cccproduccion';
            });
        }
    };
